...
local function checkMw()
  if itemAmount(3180) > 0 then return true else return false end
end

local function checkParalyze()
  if itemAmount(3165) > 0 then return true else return false end
end

local function checkPot()
  if itemAmount(3465) > 0 then return true else return false end
end

local function checkFullPot()
  if itemAmount(3465) > 7 then return true else return false end
end
...
local panels = { 
  ...
  iconDetails={text="MW Target", item = {id=3180,count=1}, switchable=false, hotkey=nil, movable=true}
  },
  {name="Paralyze Target",hotkey="",mainFunction=paralyzeTarget,checkFunction=checkParalyze,createIcon=true,
  iconDetails={text="Paralyze", item = {id=3165,count=1}, switchable=false, hotkey=nil, movable=true}
  },
  {name="1 Pot Target",hotkey="",mainFunction=potTarget,checkFunction=checkPot,createIcon=true,
  iconDetails={text="1 Pot", item = {id=3465,count=1}, switchable=false, hotkey=nil, movable=true}
  },
  {name="8 Pot Target",hotkey="",mainFunction=fullPotTarget,checkFunction=checkFullPot,createIcon=true,
  iconDetails={text="8 Pot", item = {id=3465,count=5}, switchable=false, hotkey=nil, movable=true}
  },
  {name="Catar Pots",hotkey="",mainFunction=catarPots,checkFunction=contarPots,createIcon=true,

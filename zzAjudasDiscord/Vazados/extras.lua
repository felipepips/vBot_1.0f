...
if true then
  local useId = { 34847, 1764, 21051, 30823, 6264, 5282, 20453, 20454, 20474, 11708, 11705, 
                  ...
                  5736, 6264, 31262, 31130, 31129, 6250, 6249, 5122, 30049, 7131, 7132, 7727, 25803,
                  16277, 5098, 5104, 5102, 5106, 5109, 5111, 5113, 5118, 5120, 5102, 5100, 1638, 1640, 19250,
                  3500, 3497, 3498, 3499, 2177, 17709, 1642, 23875, 1644, 5131, 5115, 28546, 6254, 28546,
                  30364, 30365, 30367, 30368, 30363, 30366, 31139, 31138, 31136, 31137, 4981, 4977, 11714, 11716,
                  7771, 9558, 9559, 20475, 2909, 2907, 8618, 31366, 1646, 1648, 4997, 22506, 8259, 27503, 27505, 27507, 31476, 31477, 31477, 31475, 31474, 8363, 33274, 33273, 33272,33271 }
  local shovelId = { 606, 593, 867, 608 }
  local ropeId = { 17238, 12202, 12935, 386, 421, 21966, 14238, 23363, 21965 }
  local macheteId = { 2130, 3696 }
  local scytheId = { 3653 }
  local adventurersstoneId = {16277}
  ...
  if settings.useAll and settings.useAll:len() > 0 then
   ...
   for _, container in pairs(containers) do
     for __, item in ipairs(container:getItems()) do
       if table.find(adventurersstoneId, item:getId()) then
         use(item)
         return
       end
     end
...
if true then
  local doorsIds = { 5007, 8265, 1629, 1632, 5129, 6252, 6249, 7715, 7712, 7714, 
                     ...
                     6264, 5122, 30049, 30042, 7727, 25803, 6907, 6258, 23875, 5733,
                     5098, 5104, 5102, 5106, 5109, 5111, 5113, 5118, 5120, 5102, 5100, 1638, 1640,
                     2177, 17709, 1644, 5131, 6254, 30364, 30365, 30363, 11714, 30366, 9558, 1646, 1648, 1644, 22506, 8259 }
  ...
  onKeyPress(function(keys)
    ...
    if keys == 'Up' or (wsadWalking and keys == 'W') then
      pos.y = pos.y - 1
    elseif keys == 'Down' or keys == 'Numpad2' or (wsadWalking and keys == 'S') then
      pos.y = pos.y + 1
    elseif keys == 'Left' or keys == 'Numpad4' or (wsadWalking and keys == 'A') then
      pos.x = pos.x - 1
    elseif keys == 'Right' or keys == 'Numpad6' or (wsadWalking and keys == 'D') then
      pos.x = pos.x + 1
    elseif wsadWalking and (keys == "Q" or keys == 'Numpad7') then
      pos.y = pos.y - 1
      pos.x = pos.x - 1
    elseif wsadWalking and (keys == "E" or keys == 'Numpad9') then
      pos.y = pos.y - 1
      pos.x = pos.x + 1
    elseif wsadWalking and (keys == "Z" or keys == 'Numpad1') then
      pos.y = pos.y + 1
      pos.x = pos.x - 1
    elseif wsadWalking and (keys == "C" or keys == 'Numpad3') then
      pos.y = pos.y + 1
      pos.x = pos.x + 1
    end
    checkForDoors(pos)
...
if true then
  ...
  local candidates = {}
  local m = macro(10, function()
    mwHot = settings.holdMwHot
    wgHot = settings.holdWgHot
    
    if not settings.holdMwall then return true end
      ...
      for i, pos in pairs(candidates) do
        ...
        if tile then
          ...
          if tile:getText() == "HOLD MW" then rune = 3180 end if tile:getText() == "HOLD WG" then rune = 3156 end
          if tile:canShoot() and not isInPz() and tile:isWalkable() and tile:getTopUseThing():getId() ~= 2130 and tile:getTopUseThing():getId() ~= 2129 then if math.abs(player:getPosition().x-tile:getPosition().x) == 0 and math.abs(player:getPosition().y-tile:getPosition().y) == 0 then return end
  ...
  onRemoveThing(function(tile, thing)
    if not settings.holdMwall then return true end
      if thing:getId() ~= 2129 and thing:getId() ~= 2130 then return true end
  ...
  onAddThing(function(tile, thing)
    if not settings.holdMwall then return true end
      if m.isOff() then return true end
      if thing:getId() ~= 2129 and thing:getId() ~= 2130 then return true end
      if tile:getText():len() > 0 then
  ...
  onKeyDown(function(keys)
    local wsadWalking = modules.game_walking.wsadWalking
    if not wsadWalking then return true end
    if not settings.holdMwall then return true end
    if m.isOff() then return true end
    if keys ~= mwHot and keys ~= wgHot then return true end
    hold = now

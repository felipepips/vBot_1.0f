-- Auto SD Target and Self UH -- OLD VERSIONS
...
macro(100, macroName, function()
  if hppercent() <= MIN_HP_TO_UH and findItem(UH_ID) then
    g_game.useWith(findItem(UH_ID),g_map.getTile(player:getPosition()):getTopUseThing())
  ...
  if not g_game.isAttacking() then
    ...
    if c and c:getHealthPercent() > MIN_HP_TARGET_TO_SD and findItem(SD_ID) then 
      g_game.useWith(findItem(SD_ID),g_map.getTile(c:getPosition()):getTopUseThing())
      delay(EXHAUSTED_TIME)
      return
    end
  end
